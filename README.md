...
3.修改 install_all_env.sh 脚本顶部配置区，自定义数据库、Redis、Grafana 密码，按需开关 LNMP/LAMP 组件  
4.执行一键环境部署  
./install_all_env.sh  
5.日常环境巡检（一键输出完整巡检报告）  
./monitor_check.sh  
6.Grafana导入监控大盘：login Grafana→仪表板→ Import，上传 docs/grafana_dashboard.json  
7.定时任务配置参考docs/crontab_task.md，实现自动巡检、自动数据库备份  
...
1.部署脚本会自动加载 conf/ 目录下预制优化配置，无需手动修改 Nginx、Tomcat、Prometheus 原生配置文件；  
2.全部账号密码、组件安装开关统一在 install_all_env.sh 顶部配置区集中修改，巡检、备份脚本同步替换密码即可适配自有环境；  
3.目录分层规范清晰，严格区分部署脚本、巡检工具、定时备份任务、服务配置、操作文档，内置 Grafana 监控面板 json 文件，开箱即用监控大盘，便于长期维护、迭代扩展；  
4.内置全套运维配套脚本，可直接落地用于 IDC 机房、私有云虚拟化批量服务器标准化初始化。  
